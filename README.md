...
|    Version    |    日付    | 履歴                                                                                                                                     |
| :-----------: | :--------: | :--------------------------------------------------------------------------------------------------------------------------------------- |
|     1.4.3     | 2022/01/30 | bugfix                                                                                                   |
|     1.4.2     | 2021/05/20 | bugfix                                                                                                   |
|     1.3.3     | 2021/04/23 | バージョン更新確認時の処理速度を改善                                                                                                     |
|     1.3.0     | 2021/04/07 | 自動制御ON/OFF時刻を曜日ごとに決められるよう修正<br>メイン画面から"O2"表示を削除                                                         |
|     1.2.0     | 2021/03/06 | 自動制御スケジュール変更時に時刻に応じて自動制御ON/OFFする機能を追加<br>濃度設定に対するバルブ開閉の補正値を変更できるよう修正<br>Bugfix |
| 1.1.1（追記） | 2021/02/24 | ACバルブ拡張モジュールの説明を追加                                                                                                       |
...
AC100V仕様のバルブを使う際は、後述のAC拡張モジュールをご使用ください。
...
## AC拡張モジュール
バルブコントローラに接続し、AC100V仕様のバルブを駆動します。

<img src="resource/ac_module.jpg" width=70%>
...
6. 開いたSDカードのフォルダ(boot)のトップの階層に、編集したwpa_supplicant.confをコピーします。ファイル名は変えないでください。
...
- バルブ開閉濃度設定：目標酸素濃度に対する、各バルブの開閉濃度の補正値を設定できます。設定値は、以下の範囲で設定してください。
  - 低酸素バルブ開＞低酸素バルブ閉
  - 空気バルブ開＜空気バルブ閉
  - 低酸素バルブ開＞空気バルブ閉
  - 低酸素バルブ閉＜空気バルブ開
...
出荷時に校正を実施していますが、お使いの環境に応じてセンサの特性が変化する場合があります。計測精度を保つため、月に1度、校正された酸素濃度計との誤差を確認し、必要であれば手順に従い校正を行ってください。（校正には別途校正された酸素濃度計が必要です）

校正手順は以下のとおりです。
- 部屋を通常の空気で満たした状態で、本システムと酸素濃度計の表示値をメモします。
- 次に、低酸素環境で、本システムと酸素濃度計の表示値をメモします。
